...
MDB_ColorPicker = ISPanelJoypad:derive("MDB_ColorPicker");

function MDB_ColorPicker:render()
...
function MDB_ColorPicker:onMouseDown(x, y)
	self.mouseDown = true
	self:onMouseMove(0, 0)
	return true
end

function MDB_ColorPicker:onMouseDownOutside(x, y)
	self:removeSelf()
    self:close()
	return true
end

function MDB_ColorPicker:onMouseMove(dx, dy)
...
function MDB_ColorPicker:onMouseUp(x, y)
...
function MDB_ColorPicker:picked2(hide)
    if hide then
        self:removeSelf()
    end
    local x = self:getMouseX()
...
function MDB_ColorPicker:removeSelf()
...
function MDB_ColorPicker:picked(hide)
	if hide then
		self:removeSelf()
	end
	if self.pickedFunc then
...
function MDB_ColorPicker:setInitialColor(initial)
	local d = 10000000
...
function MDB_ColorPicker:new(x, y)
	local buttonSize = 20
	local borderSize = 12
	local columns = 18
	local rows = 12
