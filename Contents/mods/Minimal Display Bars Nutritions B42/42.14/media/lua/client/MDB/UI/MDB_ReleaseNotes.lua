...
MDB_ReleaseNotes = ISPanel:derive("MDB_ReleaseNotes");

local MOD_VERSION = "42.14"

-- Local file I/O helpers (replaces MinimalDisplayBars.LoadFromFile / SaveToFile)
...
function MDB_ReleaseNotes:initialise()
    ISPanel.initialise(self);
    self:create();
end


function MDB_ReleaseNotes:setVisible(visible)
    --    self.parent:setVisible(visible);
    self.javaObject:setVisible(visible);
end

function MDB_ReleaseNotes:render()
    local z = 20;
...
function MDB_ReleaseNotes:create()
    local btnWid = 150
    ...
    self.cancel = ISButton:new((self:getWidth() / 2) + 5, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_btn_close"), self, MDB_ReleaseNotes.onOptionMouseDown);
...
function MDB_ReleaseNotes:updateButtons()

end

function MDB_ReleaseNotes:onOptionMouseDown(button, x, y)
    if button.internal == "CLOSE" then
        self:close()
    end
end

function MDB_ReleaseNotes:close()
    self:setVisible(false)
    self:removeFromUIManager()
end

function MDB_ReleaseNotes:new(x, y, width, height, player)
    local o = {};
...
local function okModal(_text, _centered, _width, _height, _posX, _posY, _func)
    ...
    local modal = MDB_ModalRichText:new(posX, posY, width, height, txt, false, nil, func);
...
Events.OnGameStart.Add(function()
    ...
    local data = loadFromFile("_data")
    if not data or data[1] == nil then
        data = {} end
    if data[1] == MOD_VERSION then return end

    local fContents = loadFromFile("_ReleaseNotes.txt")
    ...
    okModal(text, true, nil, nil, nil, nil,
                function()
                    ...
                    saveToFile("_data", text)

                    return
                end)

    --[[
    local update = MDB_ReleaseNotes:new(200, 200, 400, 400, getSpecificPlayer(0))
    update:initialise()
    update:instantiate()
