...
MDB_ModalRichText = ISPanelJoypad:derive("MDB_ModalRichText");

local FONT_HGT_SMALL = getTextManager():getFontHeight(UIFont.Small)

--************************************************************************--
--** MDB_ModalRichText:initialise
--**
--************************************************************************--

function MDB_ModalRichText:initialise()
	ISPanelJoypad.initialise(self);
	local btnWid = 100
	local btnHgt = math.max(25, FONT_HGT_SMALL + 3 * 2)
	local padBottom = 10
	if self.yesno then
		self.yes = ISButton:new((self:getWidth() / 2) - btnWid - 5, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_Yes"), self, MDB_ModalRichText.onClick);
		...
		self.no = ISButton:new((self:getWidth() / 2) + 5, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_No"), self, MDB_ModalRichText.onClick);
		self.no.internal = "NO";
		...
		self.ok = ISButton:new((self:getWidth() / 2) - btnWid / 2, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_Ok"), self, MDB_ModalRichText.onClick);
		self.ok.internal = "OK";
...
function MDB_ModalRichText:updateButtons()
	local btnHgt = math.max(25, FONT_HGT_SMALL + 3 * 2)
    local padBottom = 10
    if self.yesno then
...
function MDB_ModalRichText:destroy()
...
function MDB_ModalRichText:onClick(button)
	self:destroy();
...
function MDB_ModalRichText:prerender()
...
function MDB_ModalRichText:onMouseDown(x, y)
...
function MDB_ModalRichText:onGainJoypadFocus(joypadData)
...
function MDB_ModalRichText:onJoypadDown(button)
...
function MDB_ModalRichText:onJoypadDirRight()
	if self.yesno then
...
function MDB_ModalRichText:onJoypadDirLeft()
	if self.yesno then
...
function MDB_ModalRichText:update()
...
function MDB_ModalRichText:setHeightToContents()
	local btnHgt = math.max(25, FONT_HGT_SMALL + 3 * 2)
	local minHeight = self.height
	self:setHeight(minHeight)
	--self:ignoreHeightChange()
...
--************************************************************************--
--** MDB_ModalRichText:new
--**
--************************************************************************--
function MDB_ModalRichText:new(x, y, width, height, text, yesno, target, onclick, player, param1, param2)
	local o = {}
